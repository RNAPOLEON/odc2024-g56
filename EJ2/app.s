...
//------------------------------ LUNA ------------------------------//
	...
	mov x3, #50					// Radio del círculo 
	mov x2, x21					// Coordenada X (centro del círculo)
	movz x1, #150				// Coordenada Y
	movz x10, 0xD2, lsl 16
	movk x10, 0xB48C, lsl 00	// Color del círculo
	bl circle

	mov x3, #50                	// Radio del círculo 
	add x2, x21, #300		   	// Coordenada X (centro del círculo)
	movz x1, #150		   		// Coordenada Y
	bl circle

//------------------------------ PARED LUNA ------------------------------//
	
	mov x1, #0					// Coordenada X
	mov x2, #0					// Coordenada Y
	mov x3, #370				// Width (ancho) 
	mov x4, #390				// Height (alto) 
	movz x10, 0x40, lsl 16
	movk x10, 0x4040, lsl 00 	// Color de fondo	 
	bl rectangle
...
//------------------------------ MOVIMIENTOS ------------------------------//
 ...
 movz x4, 1000 //ACA DEFINO EL DELAY
...
/* 
	En end se encuentra el codigo que se utiliza para el movimiento del personaje de la muerte
	el cual se mueve con las teclas A y D (A para la izquierda, D para la derecha) 
	 Y ESPACIO para prender luces de los edificios y de la oficina.
 */
	...
	tecla_a: // Movimiento hacia la izquierda de la muerte
    cmp x19, #40
    b.lt no_mover_izquierda
    sub x19, x19, 16
    b repeat
no_mover_izquierda:
    b repeat

tecla_d: // Movimiento hacia la derecha de la muerte
    cmp x19, #420
    b.ge llego_a_420
    add x19, x19, 16
    b repeat
...
llego_a_420:
    bl animacion_sangre // Dibuja el charco de sangre cuando llega la muerte a 420
    b detener_muerte // Detiene el movimiento de la muerte


detener_muerte:
//Loop infinito para detener el movimiento de la muerte
    b detener_muerte
	
	tecla_esp:	//Prender luces de edicios y de la oficina 
    cbz x27, on
    mov x27, #0
    b repeat
    on:
